# Reference: test_13Graph.py (Graph unit tests)

The following is the Graph unit test from the topologicpy test-suite (test_13Graph.py). It is provided here as a runnable reference demonstrating typical Graph API usage (ByVerticesEdges, ByTopology, AdjacencyMatrix/List, ShortestPath, MinimumSpanningTree, VisibilityGraph, etc.).
...
---
Notes:
- Use these snippets as direct examples of Graph API usage; adapt paths and IFC-to-topology mapping as needed for your project.
- Consider adding other test files (test_01Vertex.py, test_02Edge.py, etc.) in the same manner if more references are desired.
